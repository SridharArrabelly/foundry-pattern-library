...
def check_folders_and_talk_tracks():
    ...
    if numbered != list(range(1, 13)):
        fail(f"numbered folders are not contiguous 1-12: {numbered}")
    ...
    for position, (number, folder, name, group) in enumerate(PATTERNS, start=1):
        ...
        if f"**Runs {ordinal(position)} of 12**" not in lines[2]:
...
def check_deck():
    ...
    if len(deck.slides) != 20:
        fail(f"deck has {len(deck.slides)} slides, expected 20")
    ...
    if found != [5, 6, 8, 9, 10, 11, 13, 14, 15, 17, 18, 19]:
...
def main():
    ...
    print("Consistency check passed: 12 folders, talk tracks, README surfaces, and deck align.")
